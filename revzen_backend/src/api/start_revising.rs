...
//! ## CURL Example:
//! ```bash
//! curl -X POST -F 'user_id=29' -F 'version=1' 'http://127.0.0.1:8000/api/start_relising'
//! ```
...
pub(crate) async fn api_start_revising(state: &State<StudyState>, db: RevzenDB, user_auth: Form<Client>) -> Status {
        ...
        .await
    {
        Ok(user_data) => {
            ...
            write_state.insert(user_data.id, (user_data.friendcode, user_data.username.clone()));
            Status::Ok
        },
